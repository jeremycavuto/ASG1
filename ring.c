...
void getOwnIP(int sock, char *outBuf, size_t outLen) {
  ...
  socklen_t addrLen = sizeof(ownAddress);

  if(getsockname(sock, (struct sockaddr*) &ownAddress, &addrLen) < 0){
    DieWithSystemMessage("getsockname() in getOwnIP failed");
  }
  if(inet_ntop(AF_INET, &ownAddress.sin_addr, outBuf, outLen) == NULL){
    DieWithSystemMessage("inet_ntop() in getOwnIP failed");
  }
}
